...
load_dotenv()
COMPOSITION_PATH=os.getenv("driving_licence_test_composition_path")
RURAL_URBAN_OA_PATH=os.getenv("driving_licence_test_rural_urban_OA_path")
PERSONS_DRIVING_LICENCE_PATH=os.getenv("driving_licence_test_persons_driving_licence_path")

with open("config.json") as f:
    config = json.load(f)
root = "driving_licence"
GENDERS = config["genders"]
AGE_RANGES = config[root]["age_ranges"]
HOUSEHOLD_CAR_WEIGHTS = config[root]["household_car_weights"]
GENDER_PERCENTAGES = config[root]["gender_percentages"]
...
def generate_driving_licences(df):
    ...
    for gender in GENDERS:
        df_gender = df.loc[(df['Sex'] == GENDERS[gender])]

        for count, age_range in enumerate(AGE_RANGES):
            driving_percentage = GENDER_PERCENTAGES[gender][count]

            # Create a dataframe containing only the previous selected gender
            # and one specific rural/urban area
            df_dict[f"df_{GENDERS[gender]}"] = df_gender

            # Create a dataframe containing only the previous selected gender
            # and rural/urban area with a specific range of age (e.g. 17-20)
            key = f"df_{GENDERS[gender]}_{age_range[0]}_{age_range[1]}"
            df_dict[key] = df_dict[f"df_{GENDERS[gender]}"].loc[
                ((df_dict[f"df_{GENDERS[gender]}"]['Age'] >= age_range[0])) & (
                (df_dict[f"df_{GENDERS[gender]}"]['Age'] <= age_range[1]))]
            ...
            df_dict[key + "_forced"] = (
                df_driving_licence_forced.loc)[
                    ...
                    & (df_driving_licence_forced['Sex'] == GENDERS[gender])
                ]
            ...
            for i in range(len(HOUSEHOLD_CAR_WEIGHTS)):
                # TODO: is there a better name for this variable? What is this
                #  formula?
                calculation1 = (base_df_len * HOUSEHOLD_CAR_WEIGHTS[i] *
                                driving_percentage)
...
def main():
    ...
    df_after_driving = generate_driving_licences(df_composition)

    analysis_1(df_after_driving)
    analysis_2(df_after_driving, AGE_RANGES, 1, True)
    analysis_2(df_after_driving, AGE_RANGES, 1, False)
    analysis_2(df_after_driving, AGE_RANGES, 2, True)
    analysis_2(df_after_driving, AGE_RANGES, 2, False)
    ...
    end_time = time.time()
    print(f"Driving licences assigned - completed in "
